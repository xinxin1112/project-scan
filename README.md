...
- **`--blame` 参数**：附加 git blame 作者信息（排查问题时快速定位负责人）
- 增量向量库更新（只 embed 变化的文档，基于文件 hash 比对）
- 代码块行号标注（chunk 内代码行带源码行号）
...
| `/project-scan search <query> --blame` | 搜索 + 显示代码作者（git blame） |
| `/project-scan search <query> --branch=test` | 搜索测试环境知识库 |
