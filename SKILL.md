...
3. **如果是业务后端**：从 `application.yml` 读取数据库配置，写入 `scan-config.yaml` 的 `db` 段 + 生成 `<project>/prod/kb/database-config.md`
4. 只扫描这个新项目（不动已有项目）
5. 构建该项目的向量库 + 图谱
6. 询问与已有项目的调用关系（追加到 relations）
7. 更新 `branches`、`gitnexus.repo_params` 段
...
**所有配置从 `scan-config.yaml` 读取。不要硬编码路径。不要问用户分支名。**
...
- `output_dir` — 根目录
- `projects` — 项目列表
- `branches` — 各项目 prod/test 对应的 git 分支
- `paths` — 知识库、向量库、源码的路径模板
- `gitnexus.repo_params` — 图谱查询时的 `-r` 参数
- `database` — 数据库连接信息（密码从 `password_file` 指向的文件读）
- `default_env` — 默认环境

### 使用方式

1. 读 `scan-config.yaml`
2. 从 `default_env` 确定环境（默认 test）
3. 从 `paths` 模板拼出实际路径（替换 `{project}`、`{env}`、`{output_dir}`）
4. 从 `gitnexus.repo_params.<env>.<project>` 获取图谱 `-r` 参数
5. 数据库连接：读 `<output_dir>/<project>/prod/kb/database-config.md`（每个后端项目各有一份）
   - 全局 `database` 段是 pur-center 的
   - 各项目的 `db` 段在 `projects[].db` 里
   - 密码在对应项目的 `database-config.md` 中
