...
**所有配置从 `scan-config.yaml` 读取，不要硬编码路径和密码。**

配置文件位置：`/Users/a6667/bilibili/project-scan/scan-config.yaml`

首次使用时读取配置文件获取环境信息：
```bash
cat /Users/a6667/bilibili/project-scan/scan-config.yaml
```

从中获取：
- `output_dir` — 知识库根目录
- `projects` — 项目列表（name、branch、type、source）
- `branches.prod` / `branches.test` — 各项目在 prod/test 环境对应的 git 分支
- `database` — 数据库连接信息（host/port/username/password_env）

### 环境选择

**默认用 test 环境**（日常开发都在测试分支）。不要问用户分支名，从 `branches` 段读。

### 路径规则

设 `<root>` = `scan-config.yaml` 中的 `output_dir`：

| 用途 | 路径模板 |
|------|---------|
| 知识库文档 | `<root>/<project>/<env>/kb/` |
| 向量库 | `<root>/<project>/<env>/.vector-store/` |
| 源码 prod | `<root>/.sources/<project>/` |
| 源码 test | `<root>/.sources/<project>-test/` |
| 数据库配置 | `<root>/pur-center/prod/kb/database-config.md` |
...
从 `scan-config.yaml` 的 `database` 段读取连接信息。密码从 `<root>/pur-center/prod/kb/database-config.md` 获取（`password_env` 指向的环境变量可能未设置，直接读 database-config.md 更可靠）。
