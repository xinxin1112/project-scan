...
| 项目 | prod 分支 | test 分支 | 说明 |
|------|----------|----------|------|
| pur-center | release_prod | develop | 后端核心业务 |
| srm-web | release | uat | 前端（React） |
| supplier-portal | main | develop | 供应商网关 |

**默认用 test 环境**（日常开发都在测试分支）。不要问用户分支名，直接用上表。

### 路径速查

| 用途 | 路径模板 | 示例（pur-center test） |
|------|---------|----------------------|
| 知识库文档 | `<root>/<project>/<env>/kb/` | `/Users/a6667/bilibili/project-scan/pur-center/test/kb/` |
| 向量库 | `<root>/<project>/<env>/.vector-store/` | `/Users/a6667/bilibili/project-scan/pur-center/test/.vector-store/` |
| 源码（图谱用） | `<root>/.sources/<project>-<env>/` | `/Users/a6667/bilibili/project-scan/.sources/pur-center-test/` |
| 数据库配置 | `<root>/pur-center/prod/kb/database-config.md` | 含 host/port/user/password |

`<root>` = `/Users/a6667/bilibili/project-scan`

**注意：`.sources/` 是源码目录（给 GitNexus 图谱用），不是知识库。**
...
# 向量搜索（prod）
node /Users/a6667/.claude/skills/project-scan/scripts/vector-search.js "关键词" --project=pur-center --branch=prod --top=5
...
# 数据库连接
mysql -h 10.150.18.39 -P 5517 -u purchase -pJAcp9GnzqEUxuayx4xTVHydnSoyYbIkz purchase
```
...
# prod 分支源码
grep -ri "关键词" /Users/a6667/bilibili/project-scan/.sources/pur-center/app/ --include="*.java" -l
```
