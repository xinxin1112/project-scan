...
**不要问用户分支名** — 直接用上表。用户说"测试环境"就用 test 列的分支，说"生产"就用 prod 列的。
...
├── scan-config.yaml
├── .sources/                          ← 源码（git clone/worktree），不是知识库
│   ├── pur-center/                    ← prod 源码
│   ├── pur-center-test/               ← test 源码（worktree）
│   ├── srm-web/
│   ├── srm-web-test/
│   ├── supplier-portal/
│   └── supplier-portal-test/
├── pur-center/
│   ├── prod/kb/                       ← prod 知识库文档
...
```bash
# 搜 prod 知识库
node scripts/vector-search.js "关键词" --project=pur-center --branch=prod

# 搜 test 知识库
node scripts/vector-search.js "关键词" --project=pur-center --branch=test
```
...
需要连数据库时，读取知识库中的配置文件：
```
/Users/a6667/bilibili/project-scan/pur-center/prod/kb/database-config.md
```
