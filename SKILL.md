...
首次使用时读取配置文件获取 `output_dir`、项目列表、分支映射：
```bash
...
从 `scan-config.yaml` 的 `branches` 段读取。**默认用 test 环境**（日常开发都在测试分支）。不要问用户分支名。
...
读取 `<root>/pur-center/prod/kb/database-config.md` 获取连接信息，不要硬编码密码。
